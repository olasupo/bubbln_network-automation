...
def get_router_interface_configuration(protocol_count, interface_count):
    ...
    for _ in range(protocol_count):
        while True:
            protocol = input_with_history("Please input the network protocol to configure (Choice: OSPF, EIGRP): ").lower()
            if protocol.lower() in ['ospf', 'eigrp']:
                break
            else:
                animate_message("Sorry, thats an invalid input! Please enter either 'OSPF' or 'EIGRP'\n")
            ...
            for k in range(num_networks):
                while True:
                    ...
                    if validate_cidr_network(network):
                        ...
                        animate_message("Thats an invalid input! Please enter a valid CIDR network in the format x.x.x.x/y.")
                network_address = convert_cidr_network_wildcard(network)
                networks.append(network_address)
            router_interface_config.append({'protocol': protocol, 'area': area, 'process_id': process_id, 'networks': networks})
            ...
            for k in range(num_networks):
                while True:
                    ...
                    if validate_cidr_network(network):
                        ...
                        animate_message("Sorry, thats an invalid input! Please enter a valid CIDR network in the format x.x.x.x/y.\n")
                network_address = convert_cidr_network_wildcard(network)
                networks.append(network_address)
